...
class LearningRateWriter(LossWriter):
    def __init__(self, write_fn):
        super().__init__(write_fn)
        self.overall_batch = 0
    
    def on_train_batch_end(self, batch, logs=None):
        if callable(self.model.optimizer.lr):
            lr = self.model.optimizer.lr(self.overall_batch).numpy()
            self.write_fn("learning_rate", lr, self.overall_batch)
